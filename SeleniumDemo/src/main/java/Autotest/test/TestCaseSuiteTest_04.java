...
public class TestCaseSuiteTest_04 extends BaseTest {
    ...
    public TestCaseSuiteTest_04() {
        setSheetName(TestCaseSuiteTest_04.class.getSimpleName());
    }

    @Test(description = "NC001: Name cannot be empty")
    public void NC001_Name_cannot_be_empty() {
        HashMap<String, String> data = getJsonTestData("NC001");
        Manager objManager = objLogin.login_Guru99_with(data.get("UserId"), data.get("password"));
        objNewCustomer = objManager.leftMenu().move_to_New_Customer();
        objNewCustomer.input_Customer_Name(data.get("CustomerName"));
        assertTrue(objNewCustomer.should_Show_Customer_Name_Error_Message(data.get("CustomerErrorMessage")));
    }

    @Test(description = "NC002: Name cannot be numeric")
    public void NC002_Name_cannot_be_numeric() {
        HashMap<String, String> data = getJsonTestData("NC002");
        objNewCustomer.input_Customer_Name(data.get("CustomerName"));
        assertTrue(objNewCustomer.should_Show_Customer_Name_Error_Message(data.get("CustomerErrorMessage")));
    }
    ...
    public void NC003_Name_cannot_have_special_characters() {
        HashMap<String, String> data = getJsonTestData("NC003");
        objNewCustomer.input_Customer_Name(data.get("CustomerName"));
        assertTrue(objNewCustomer.should_Show_Customer_Name_Error_Message(data.get("CustomerErrorMessage")));
    }

    @Test(description = "NC004: Name cannot have first character as blank space")
    public void NC004_Name_cannot_have_first_character_as_blank_space() {
        HashMap<String, String> data = getJsonTestData("NC004");
        objNewCustomer.input_Customer_Name(data.get("CustomerName"));
        assertTrue(objNewCustomer.should_Show_Customer_Name_Error_Message(data.get("CustomerErrorMessage")));
    }
